...
# 시간효율 개선
# 군집 정보 2차원배열 말고 1차원배열로 관리
# 병합을 위한 이동된 군집은, dictionary에 저장
"""
direction = (None, (-1, 0), (1, 0), (0, -1), (0, 1))
opposite = [None, 2, 1, 4, 3]
...
def move():
    for y, x, cnt, d in groups:
        ...
        # 이동 상태 반영
        new_area[ny * n + nx] = new_area.get(ny * n + nx, []) + [(cnt, d)]
 
 
# 두 군집 이상 모인 곳에 대해 병합시키는 함수
def merge():
    for yx in new_area.keys():
        y, x = divmod(yx, n)
        if len(new_area[y * n + x]) > 1:  # 2 이상이라면
            new_area[y * n + x].sort(reverse=True)  # 개수 내림차순 정렬
            new_area[y * n + x] = [(sum([ar[0] for ar in new_area[y * n + x]]), new_area[y * n + x][0][1])]  # [(개수 합, 개수 가장 많은 군집의 방향)]
        new_groups.append((y, x, new_area[y * n + x][0][0], new_area[y * n + x][0][1]))
...
for test in range(1, t + 1):
    ...
    # m 시간 동안 반복 수행
    for _ in range(m):
        new_area = dict()  # 이동된 군집 저장할 dictionary
        move()   # 군집 이동
